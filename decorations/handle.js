...
var concatMatrix = function(m1, m2) {
...
var place = function (obj, canvas, ctx, matrix, ctrans, blendMode, frame, ratio, time) {
    ...
    if (blendMode > 1) {
        obj(ctx,new cxform(0,0,0,0,255,255,255,255),frame,ratio,time);
    } else {
        obj(ctx,ctrans,frame,ratio,time);
    }
    if (blendMode > 1) {
...
var drawMorphPath = function(ctx, p, ratio, doStroke, scaleMode) {
    var parts = p.split(" ");
...
var useRatio = function(v1, v2, ratio) {
    return v1 * 1 + (v2 - v1) * ratio / 65535;
}

var drawPath = function(ctx, p, doStroke, scaleMode) {
    var parts = p.split(" ");
    var len = parts.length;
    if (doStroke) {
        for (var i = 0; i < len; i++) {
...
var shadeColor = function(rgb, gray) {
    rgb = rgb || [0,0,0];
    let diff = 2*(gray[0]/255);
    gray[0] = clamp(rgb[0]*diff,0,255);
    gray[1] = clamp(rgb[1]*diff,0,255);
    gray[2] = clamp(rgb[2]*diff,0,255);
    return gray;
}

var required = {}

module.exports = (newCanvas, fileName, colorPalette) => {
    createCanvas = newCanvas;
    if (!required[fileName])
        required[fileName] = require(fileName);
    return required[fileName](newCanvas, concatMatrix, enhanceContext, cxform, place, tocolor, drawMorphPath, useRatio, drawPath, shadeColor, colorPalette.split(',').map(hexToRgb));
}